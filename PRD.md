...
5.3.1 多 Agent 会话管理（Multi-Agent Session Management）

背景
随着用户同时处理多个任务的需求增加，需要支持多个 Agent 会话并行运行，并在侧边栏清晰展示各会话的状态。

需求
	•	会话状态追踪：
		- 每个会话维护独立的状态：idle（空闲）、running（运行中）、completed（已完成）、failed（失败）、interrupted（已中断）
		- 状态变更实时同步到数据库和 UI
	•	会话命名优化：
		- 会话名称自动设置为用户发送的第一条消息（截断至 30 字符）
		- 支持用户手动重命名覆盖自动名称
	•	时间戳格式：
		- 使用绝对时间格式显示，如"1月6日 14:31"
		- 当年显示"月日 时:分"，跨年显示"年月日 时:分"
	•	任务进度追踪（可选）：
		- 存储任务列表 JSON，支持进度展示
...
interface SessionMetadata {
  ...
  firstMessage: string | null  // 第一条用户消息
  tasksJson: string | null     // 任务列表 JSON
}
```

API 扩展
	•	update_session_status(session_id, status) - 轻量级状态更新
	•	set_session_first_message(session_id, message) - 设置首条消息
	•	update_session_tasks(session_id, tasks_json) - 更新任务列表

验收
	•	运行中的会话在列表中优先显示并有动画指示
	•	会话状态变更在 500ms 内反映到 UI
	•	首条消息自动成为会话名称（无需用户手动命名）
	•	时间戳使用清晰的绝对格式
